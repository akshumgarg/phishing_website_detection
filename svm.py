...
Algorithm: Linear SVM with Hinge Loss + L2 Regularization
...
# ══════════════════════════════════════════════════════════════
# SVM CLASS (from scratch — Linear SVM with SGD)
# ══════════════════════════════════════════════════════════════

class SVMScratch:
    """
    Linear Support Vector Machine using Hinge Loss + L2 Regularization.

    Decision function:  f(x) = w · x + b
    Hinge Loss:         max(0, 1 - y_i * f(x_i))
    Objective:          (1/n) * Σ max(0, 1 - y_i*(w·x_i + b)) + λ * ||w||²
        ...
        for iteration in range(self.n_iterations):
            ...
            for idx in indices:
                ...
                if condition >= 1:
                    ...
                    # Hinge loss = 0 for this sample
                else:
                    # Misclassified or within margin
                    for j in range(n_features):
                        self.weights[j] -= self.lr * (2 * self.lambda_param * self.weights[j] - yi * xi[j])
                    self.bias -= self.lr * (-yi)
                    # Hinge loss contribution
                    epoch_loss += (1 - condition)

            # Average hinge loss + regularization term
    ...
    def predict_binary(self, X):
        """Return 0/1 predictions (for metric computation)."""
...
def confusion_matrix_vals(y_true, y_pred):
    """Returns TP, TN, FP, FN. Expects 0/1 labels."""
...
def roc_auc_score(y_true_01, decision_scores):
    """Compute ROC-AUC using decision scores and trapezoidal rule."""
...
if __name__ == "__main__":
    ...
    X = []
    y_binary = []  # 0/1 labels
    y_svm = []     # -1/+1 labels (for SVM training)
    skipped = 0
    for row in data:
        try:
            features = [float(row[i]) for i in feature_indices]
            is_phishing = row[status_idx].strip().lower() == "phishing"
            X.append(features)
            y_binary.append(1 if is_phishing else 0)
            y_svm.append(1 if is_phishing else -1)
    ...
    # ── 3. Train/Test Split ──────────────────────────────────
    # Split using binary labels, then convert train labels for SVM
    X_train, X_test, y_train_bin, y_test_bin = train_test_split(X, y_binary, test_size=0.2, random_seed=42)

    # Convert to SVM labels (-1/+1)
    ...
    print(f"   Learning rate: 0.001  |  Lambda (C⁻¹): 0.01  |  Epochs: 1000\n")
    ...
    # ── 6b. Predict on TRAIN set (for comparison) ────────────
    ...
    # Margin stats
    margins = [y_test_svm[i] * decision_scores[i] for i in range(n_test)]
    margin_mean = mean(margins)
    margin_min = min(margins)
    margin_max = max(margins)
    support_vectors_approx = sum(1 for m in margins if abs(m) <= 1.0)
    ...
    overfit_flag = "Potential overfitting!" if (acc_train - acc) > 0.05 else "No significant overfitting"
    print(f"\n   Assessment: {overfit_flag}")

    print("\n┌─────────────────────────────────────────────────────────────┐")
    print("│  G. SVM-SPECIFIC: MARGIN ANALYSIS                          │")
    print("└─────────────────────────────────────────────────────────────┘")
    ...
    print(f"   {'Samples within margin (|m|<=1)':<35s} {support_vectors_approx:>10d}")
    print(f"   {'% within margin':<35s} {support_vectors_approx/n_test*100:>9.2f}%")
    ...
    max_loss = max(l for _, l in sampled) if sampled else 1
    ...
    print("└─────────────────────────────────────────────────────────────┘")
    print(f"   Algorithm:        Linear SVM (Support Vector Machine)")
    print(f"   Kernel:           Linear")
    print(f"   Loss Function:    Hinge Loss + L2 Regularization")
    print(f"   Optimization:     Stochastic Gradient Descent (SGD)")
    ...
    with open(report_file, "w") as f:
        ...
        f.write(f"  Legitimate:        {n_legitimate} ({n_legitimate/len(y_binary)*100:.1f}%)\n")
        ...
        f.write("H. SVM MARGIN ANALYSIS\n")
        f.write("-" * 40 + "\n")
        f.write(f"  Mean Margin:              {margin_mean:.4f}\n")
        f.write(f"  Min Margin:               {margin_min:.4f}\n")
        f.write(f"  Max Margin:               {margin_max:.4f}\n")
        f.write(f"  Samples within margin:    {support_vectors_approx}\n")
        f.write(f"  % within margin:          {support_vectors_approx/n_test*100:.2f}%\n\n")
        ...
        f.write(f"\nL. MODEL PARAMETERS\n")
        f.write("-" * 40 + "\n")
        f.write(f"  Algorithm:         Linear SVM\n")
        f.write(f"  Kernel:            Linear\n")
        f.write(f"  Loss Function:     Hinge Loss + L2 Regularization\n")
        f.write(f"  Optimization:      SGD\n")
