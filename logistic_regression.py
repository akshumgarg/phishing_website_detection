...
if __name__ == "__main__":
    ...
    print("   Learning rate: 0.1  |  Iterations: 1000\n")
    ...
    prec_ph = precision(tp, fp)
    rec_ph = recall(tp, fn)
    f1_ph = f1_score(prec_ph, rec_ph)
    ...
    overfit_flag = (
        "⚠️  Potential overfitting!" if (acc_train - acc) > 0.05 else "✅ No significant overfitting"
    )
    ...
    print("└─────────────────────────────────────────────────────────────┘")
    print("   Algorithm:        Logistic Regression (Binary)")
    ...
    with open(report_file, "w") as f:
        ...
        f.write(
            f"  Reduction:    {(1 - model.loss_history[-1] / model.loss_history[0]) * 100:.2f}%\n\n"
        )
        ...
        f.write("\nK. MODEL PARAMETERS\n")
        f.write("-" * 40 + "\n")
        f.write("  Algorithm:         Logistic Regression (Binary)\n")
