"""
Logistic Regression for Phishing Website Detection
===================================================
    ...
    def fit(self, X, y):
        ...
        for iteration in range(self.n_iterations):
            ...
            for i in range(n_samples):
                z = dot_product(X[i], self.weights) + self.bias
            ...
            if (iteration + 1) % 100 == 0 or iteration == 0:
                print(f"   Iteration {iteration+1:>4d}/{self.n_iterations}  |  Loss: {loss:.6f}")

    def predict_proba(self, X):
        """Return probability of class 1."""
        probs = []
        for row in X:
            z = dot_product(row, self.weights) + self.bias
...
if __name__ == "__main__":
    print("=" * 60)
    print("  LOGISTIC REGRESSION — PHISHING WEBSITE DETECTION")
    ...
    # ── 5. Train Model ───────────────────────────────────────
    print("\n🏋️  Training Logistic Regression (Gradient Descent)...")
    print(f"   Learning rate: 0.1  |  Iterations: 1000\n")
    ...
    tp_tr, tn_tr, fp_tr, fn_tr = confusion_matrix(y_train, y_train_pred)
    n_test = len(y_test)
    ...
    auc_train = roc_auc(y_train, y_train_prob)
    ...
    # ── Feature Importances ──────────────────────────────────
    print("\n┌─────────────────────────────────────────────────────────────┐")
    print("│  G. TOP 15 FEATURE IMPORTANCES                             │")
    print("└─────────────────────────────────────────────────────────────┘")
    ...
    for idx, loss_val in sampled:
        ...
        bar = "█" * bar_len
        print(f"   Iter {idx+1:>4d} │ {bar:<{max_bar}s} │ {loss_val:.6f}")
    ...
    print("└─────────────────────────────────────────────────────────────┘")
    print(f"   Algorithm:        Logistic Regression (Binary)")
    ...
    print("\n" + "=" * 65)
    print("  ✅ LOGISTIC REGRESSION COMPLETE (No external libraries!)")
    print("=" * 65)

    # ══════════════════════════════════════════════════════════
    # WRITE REPORT TO FILE
    # ══════════════════════════════════════════════════════════
    report_file = "logistic_regression_report.txt"
    with open(report_file, "w") as f:
        f.write("=" * 70 + "\n")
        f.write("  LOGISTIC REGRESSION PERFORMANCE REPORT\n")
        ...
        f.write("H. TOP 15 FEATURE IMPORTANCES\n")
        f.write("-" * 60 + "\n")
        ...
        f.write(f"\nI. ALL FEATURE WEIGHTS ({len(feature_names)} features)\n")
        f.write("-" * 60 + "\n")
        for rank, (fname, w) in enumerate(feature_weight_pairs, 1):
            f.write(f"  {rank:<4d} {fname:<30s} {w:>+12.6f}\n")

        f.write(f"\nJ. TRAINING LOSS HISTORY\n")
        f.write("-" * 40 + "\n")
        f.write(f"  Initial Loss: {model.loss_history[0]:.6f}\n")
        f.write(f"  Final Loss:   {model.loss_history[-1]:.6f}\n")
        f.write(f"  Reduction:    {(1 - model.loss_history[-1]/model.loss_history[0])*100:.2f}%\n\n")
        for i in range(0, len(model.loss_history), 100):
            f.write(f"  Iteration {i+1:>4d}: Loss = {model.loss_history[i]:.6f}\n")

        f.write(f"\nK. MODEL PARAMETERS\n")
        f.write("-" * 40 + "\n")
        f.write(f"  Algorithm:         Logistic Regression (Binary)\n")
        f.write(f"  Optimization:      Gradient Descent\n")
        f.write(f"  Learning Rate:     {model.lr}\n")
        f.write(f"  Iterations:        {model.n_iterations}\n")
